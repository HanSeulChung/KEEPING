...
export const notificationApi = {
  ...
  // 알림 목록 조회
  getNotificationList: async (ownerId: number): Promise<NotificationAPI.NotificationResponseDto[]> => {
    try {
  ...
  // 읽지 않은 알림 조회
  getUnreadNotifications: async (ownerId: number): Promise<NotificationAPI.NotificationResponseDto[]> => {
    try {
      const response = await apiClient.get<NotificationAPI.OwnerNotificationListResponse>(
  ...
  subscribeNotifications: (ownerId: number): EventSource => {
    const eventSource = new EventSource(`/notifications/subscribe/owner/${ownerId}`)
    return eventSource
  },
